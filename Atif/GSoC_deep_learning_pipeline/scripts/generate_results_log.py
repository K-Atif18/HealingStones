...
def section_dry_run() -> str:
    """Fold-1 dry-run outcome (timing + plumbing gate) and the mining
    optimization it motivated. Recorded here because it changed a headline
    number (per-epoch wall clock) and closed Deviation 2's open ratio."""
    return """## (g) Fold-1 dry run + mining optimization
...
def section_jitter_deviation() -> str:
    """Section (i): the jitter-seed defect and its fix (regularization deviation)."""
    return """## (i) Deviation 4 -- jitter augmentation was inert (constant seed)
