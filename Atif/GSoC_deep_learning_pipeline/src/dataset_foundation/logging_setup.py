"""Structured logging setup for the Dataset Foundation pipeline.
...
Fragment_ID (where applicable), and a success status (Req 9.4); on failure the
same fields are emitted with a failure status (Req 9.7).
"""
...
ROOT_LOGGER_NAME = "dataset_foundation"
...
def configure_logging(
    ...
    *,
    ...
    formatter on the ``dataset_foundation`` logger. The call is idempotent: a
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    ...
    Args:
        name: Optional component name (e.g. ``"aligner"``). When omitted the
            package root logger is returned. Fully-qualified names already under
            the package root are returned unchanged.

    Returns:
        A :class:`logging.Logger` namespaced beneath ``dataset_foundation``.
    """
...
def log_stage(
    ...
    **fields: Any,
    ...
    ``ERROR`` by default (Req 9.4, 9.7); an explicit ``level`` overrides this.

    Args:
        logger: The logger to emit through (typically from :func:`get_logger`).
        stage: The pipeline stage name (e.g. ``"align"``, ``"normalize"``).
        ...
        **fields: Arbitrary extra structured fields (e.g. ``inlier_rmse_mm``).
    """
