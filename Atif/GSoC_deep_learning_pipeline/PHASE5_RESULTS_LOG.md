...
- Generated: `2026-07-25T17:04:00.314429+00:00`
- Source JSON `run_timestamp`: `2026-07-23T16:53:22.637596+00:00`
- Git commit at generation: `5dad27a`
...
## (j) Condition 3 was a tautology -- the negative result (2026-07-25)
...
## (d) Encoder results (Phase 5A, 6-fold LOFO)

Provenance: `phase5a_runs_jitter/eval_encoder.json` (F1), `phase5a_runs_6fold/fold{2,3,4}/eval.json`, produced by `scripts/export_and_evaluate.py`. All numbers read from those JSONs.

### Conditions 1-2 -- retrieval (paired mAP diff, encoder - FPFH)

| fold | enc mAP | FPFH mAP | paired diff [95% CI] | enc P@1 | FPFH P@1 |
| --- | --- | --- | --- | --- | --- |
| F1 | 0.0986 | 0.1053 | -0.0067 [-0.0095, -0.0039] | 0.1414 | 0.1676 |
| F2 | 0.1069 | 0.1391 | -0.0322 [-0.0358, -0.0286] | 0.1279 | 0.1938 |
| F3 | 0.0881 | 0.1101 | -0.0220 [-0.0242, -0.0199] | 0.0995 | 0.1693 |
| F4 | 0.1115 | 0.1280 | -0.0166 [-0.0200, -0.0131] | 0.1085 | 0.1838 |

> All four paired CIs exclude zero on the LOSING side; P@1 below FPFH on all four. **Conditions 1-2 FAILED, 0/4.**

### Condition 3 -- VALID encoder-mined easy-vs-hard (thesis)
...
> hard-AUC BELOW CHANCE on all four folds. **Condition 3 NOT MET.** The FPFH-mined version of this metric was a tautology (section j).

### Condition 6 -- held-out fragment-ID self-retrieval

| fold | enc self-retr | FPFH self-retr | base rate |
| --- | --- | --- | --- |
| F1 | 0.519 | 0.895 | 0.147 |
| F2 | 0.489 | 0.876 | 0.147 |
| F3 | 0.518 | 0.935 | 0.147 |
| F4 | 0.488 | 0.850 | 0.147 |

> Encoder ~half FPFH's identity leakage across folds -- improved, but least thesis-critical.
...
- **1-2 (retrieval):** FAILED 0/4 (table above).
- **3 (compatibility/thesis):** NOT MET, unanimously (valid test above).
- **4 (neighbour contact gap):** not separately re-run on the encoder; moot given 1-3.
- **5 (pose gate):** PASSED (`max_dev=0.0`, re-verified each run).
- **6 (fragment-ID / boundary):** fragment-ID leakage improved vs FPFH; does not rescue 1-3.

Per the pre-registered rule (condition 1 fails, 3 fails): this is NOT a Level-3 success. Honest conclusion in section (j): a learned PointNet contrastive encoder on ~100-point/8mm patches with symmetric co-membership labels on one 7-fragment artifact does not beat FPFH at interface-association retrieval and does not learn complementarity. A clean, diagnosed NEGATIVE result -- see section (j).
